...
import org.source.controllers.ClientDetailsPanelController;
import org.source.service.CacheService;
import org.source.views.base.BasePanel;

public class ClientDetailsPanel extends BasePanel {
	...
	private void initComponents() {
		...
		m_topText = new JLabel("Информация о пользователе", SwingConstants.CENTER);
		...
		m_removeButton = new JButton("Удалить пользователя");
		m_removeButton.setPreferredSize(new Dimension(300, 50));
		m_removeButton.setFont(m_fontButtons);
	...
	private void buildLayout() {
		...
		m_componentsPanel.add(m_removeButton);
	...
	public void actionPerformed(ActionEvent event) {
		...
		if(object == m_ordersButton) {
			ClientDetailsPanelController.getInstance().openHistory();
		} else if(object == m_removeButton) {
			ClientDetailsPanelController.getInstance().removeUser();
		} else if(object == m_backButton) {
			ClientDetailsPanelController.getInstance().back();
		}
	}
}
