...
import org.source.controllers.AuthorizationClientPanelController;
import org.source.service.CacheService;
import org.source.views.base.BasePanel;

public class AuthorizationClientPanel extends BasePanel {
	...
	public AuthorizationClientPanel() {
		super();
		initComponents();
		buildLayout();
		addEvents();
	...
	private void initComponents() {
		...
		m_topText = new JLabel("Авторизация", SwingConstants.CENTER);
		m_topText.setPreferredSize(new Dimension(450, 30));
		m_topText.setFont(m_fontButtons);
		
		m_infoText = new JLabel("<html>Войдите в свой аккаунт. Если вы " +
				"не зарегистрированы, то можете пройти регистрацию.<html>");
		m_infoText.setVerticalAlignment(SwingConstants.TOP);
		m_infoText.setPreferredSize(new Dimension(450, 50));
	...
	public void actionPerformed(ActionEvent event) {
		...
		if(object == m_authorizationButton) {
			...
			AuthorizationClientPanelController.getInstance().authorization(this);
		} else if(object == m_registrationButton) {
			AuthorizationClientPanelController.getInstance().registration();
		} else if(object == m_backButton) {
			AuthorizationClientPanelController.getInstance().back();
		}
	}
}
