...
import org.source.controllers.ControlProductPanelController;
import org.source.models.Product;
import org.source.service.CacheService;
import org.source.views.base.BasePanel;

public class ControlProductsPanel extends BasePanel {
	...
	private void initComponents() {
		...
		m_listProductsScroll.setPreferredSize(new Dimension(450, 200));
		
		m_descriptionButton = new JButton("Информация о товаре");
	...
	public void actionPerformed(ActionEvent event) {
		...
		if(object == m_searchButton) {
			...
			ControlProductPanelController.getInstance().search(this);
			...
			ControlProductPanelController.getInstance().description();
		} else if(object == m_addButton) {
			ControlProductPanelController.getInstance().addProduct();
		} else if(object == m_backButton) {
			ControlProductPanelController.getInstance().back();
		}
	}
}
